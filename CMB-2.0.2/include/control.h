...
#if defined(__QNX__)
	SND_CTL_EVENT_CM   = 1,                          /* QNX Extension (Concurrency Management)   */
	SND_CTL_EVENT_AFM  = 2,                          /* QNX Extension (Acoustic Function Module) */
	SND_CTL_EVENT_LAST = SND_CTL_EVENT_AFM
#else
...
#if defined(__QNX__)
/* Translate snd_ctl_event_type_t values into a bitmask for use with snd_event_filter_t */
#define SND_CTL_EVENT_TYPE_MASK(x) (1 << (x - SND_CTL_EVENT_ELEM))
#endif
...
#if defined (__QNX__)
#define SND_CTL_NAME_PLAYBACK_CHMAP "Playback Channel Map"
#define SND_CTL_NAME_CAPTURE_CHMAP  "Capture Channel Map"
#endif
...
#if defined (__QNX__)
int snd_ctl_set_event_filter(snd_ctl_t const * const ctl, snd_event_filter_t const * const filter);
#endif
...
#if defined (__QNX__)
char const * snd_ctl_event_cm_get_output_id(snd_ctl_event_t const * const obj);
int snd_ctl_event_cm_get_cmd(snd_ctl_event_t const * const obj);
int snd_ctl_cm_status_read(snd_ctl_t const * const ctl, char const * const cm_output, snd_pcm_cm_priority_status_t **info);
int snd_ctl_event_afm_get_cmd(snd_ctl_event_t const * const obj);
int snd_ctl_event_afm_get_data(snd_ctl_event_t const * const obj);
int snd_ctl_event_afm_get_channel(snd_ctl_event_t const * const obj);
uint64_t snd_ctl_event_afm_get_tstamp(snd_ctl_event_t const * const obj);
#endif
...
#if defined (__QNX__)
snd_pcm_chmap_t * snd_hctl_elem_get_channel_map(snd_hctl_elem_t const * const elem);
int snd_hctl_elem_add_chmap(snd_hctl_elem_t * const helem);
#endif
