...
#if defined(__QNX__)
#define SM_CAP_PBALANCE      (1<<14)
#define SM_CAP_PFADE         (1<<15)
#define SM_CAP_CBALANCE      (1<<16)
#define SM_CAP_CFADE         (1<<17)
#define SM_CAP_INTERNAL_RAMP (1<<18)
#define SM_CAP_SUBCHANNEL    (1<<19)
#endif
...
#if defined (__QNX__)
	char const * (*get_channel_name)(snd_mixer_elem_t const * const elem, snd_mixer_selem_channel_id_t const channel, int const dir);
	int (*get_balance)(snd_mixer_elem_t const * const elem, int const dir, long * const value, long * const min, long * const max);
	int (*set_balance)(snd_mixer_elem_t * const elem, int const dir, long const value);
	int (*get_fade)(snd_mixer_elem_t const * const elem, int const dir, long * const value, long * const min, long * const max);
	int (*set_fade)(snd_mixer_elem_t * const elem, int const dir, long const value);
#endif
};
