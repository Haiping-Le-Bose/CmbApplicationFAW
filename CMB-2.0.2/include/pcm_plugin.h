 ...
 */
#if defined (__QNX__)
int snd_pcm_hw_open(snd_pcm_t **pcmp, char const * const name, char const * const pathname,
                    int const card, int const device, int const subdevice, snd_pcm_stream_t const stream,
                    int const mode, int sync_ptr_ioctl);
#else
