...
int32_t BAF_command(BAF* baf, BAF_Message* req, BAF_Message* rsp)
{
    ...
    int32_t status;
    uint32_t src, dst, handle;
    CART_MESSAGE_TYPE type = CART_Message_get_type(req);
    uint32_t jm_mask = this_private->resource.mask;


    src=CART_Message_get_src(req);
    dst=CART_Message_get_dst(req);
    Message_req.module_id = dst & jm_mask; // masked dst is JModule ID
    Message_req.submodule_id = 0; //(req->id >> 16) & 0xff;
    Message_req.message_id = CART_Message_get_id(req);
    Message_req.message_size = CART_Message_get_size(req);
    Message_req.message_data = req->data;
    handle=CART_Message_get_handle(req);

    if( rsp != NULL ) {
        Message_rsp.message_size = CART_Message_get_size(rsp);	
    ...
    if (Message_req.message_id == CART_MESSAGE_ID_DEBUG) {
        Message_req.message_id = req->data[0];
        Message_req.submodule_id = (req->id >> 16) & 0xff;
    }
    status = JFramework_command(&this_private->framework, &Message_req, &Message_rsp);
    if (Message_req.message_id == CART_MESSAGE_ID_NOTIFICATION_POLL) {
        handle = ((uint32_t*)Message_rsp.message_data)[TALARIA_HANDLE_OFFSET]; // for notifications use Talaria handle
        src = ((uint32_t*)Message_rsp.message_data)[TALARIA_HANDLE_OFFSET] >> 16; // get source from handle
                CART_Message_set_type(rsp, CART_MESSAGE_TYPE_NOTIFY);
    }

    if( rsp != NULL ) {
        CART_Message_set_id(rsp,Message_rsp.message_id);
        ...
        if(status > 0) {
             ...
             else if (Message_req.message_id == CART_MESSAGE_ID_NOTIFICATION_POLL){
                CART_Message_set_type(rsp, CART_MESSAGE_TYPE_NOTIFY);
                CART_Message_set_size(rsp, status);
             }
             else {
