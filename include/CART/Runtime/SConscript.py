...
local_env = env.Clone(
    CPPPATH=['.','BAF','JSched','Audio_Graph','CART'])
...
if 'runtime' in globals() and ('Context' in runtime) and (runtime['Context'] == True):
    local_env.Append(CPPDEFINES='BAF_CALLBACK_CONTEXT')
...
if 'option' in globals() and option:
    if 'Print' in option and option['Print']:
        local_env.Append(CPPDEFINES='BAF_PRINTF_SUPPORTED')
    if 'OJ3Support' in option and (option['OJ3Support'] == False):
        local_env.Append(CPPDEFINES='BAF_REMOVE_OJ3_SUPPORT')
    if 'PrimeBuffer' in option and option['PrimeBuffer']:
        local_env.Append(CPPDEFINES='BAF_PRIME_BUFFER')
    if 'UseSchedule' in option and option['UseSchedule']:
        local_env.Append(CPPDEFINES='BAF_USE_SCHEDULE')
    if 'JSched' in option and option['JSched']:
        local_env.Append(CPPDEFINES='BAF_PRIORITY_SCHEDULER')
        source = source + [ 'JSched/JExec.c', 'JSched/JSched.c']
    
    if 'Connection' in option and option['Connection']:
        local_env.Append(CPPDEFINES='BAF_CONNECTION')
        source = source + ['Connection.c']
    if 'Semaphore' in option and option['Semaphore']:
        local_env.Append(CPPDEFINES='USE_BAF_SEMAPHORE')
    elif 'Connection' in option and option['Connection']:
        source = source + [ 'Semaphore.c']
    if 'Logger' in option and option['Logger']:
        local_env.Append(CPPDEFINES='BAF_LOGGER')
        if 'BAFStatsLogger' in option and option['BAFStatsLogger']:
            local_env.Append(CPPDEFINES='BAF_STATSLOGGER')
